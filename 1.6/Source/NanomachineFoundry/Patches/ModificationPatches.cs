...
namespace NanomachineFoundry.Patches
{
    ...
    [HarmonyPatch]
    public static class AbilityCanCastPatch
    {
	    [HarmonyReversePatch]
	    [HarmonyPatch(typeof(Ability), nameof(Ability.CanCast), MethodType.Getter)]
	    [MethodImpl(MethodImplOptions.NoInlining)]
	    public static AcceptanceReport CallBaseAbility(Ability instance)
	    {
    ...
    public class ModificationPatches
    {
        ...
        public static void ApplyPatches(Harmony harmony)
        {
            ...
            harmony.Patch(AccessTools.Method(typeof(FloatMenuMakerMap), "GetOptions"),
	            new HarmonyMethod(typeof(ModificationPatches), nameof(PreOrdersGiven)));
            // Log.Message("-80");
        ...
        private static bool PrePsycastCanCast(ref AcceptanceReport __result, Psycast __instance)
        {
            if (!IsPsycastCached(__instance, out ModificationWorker_PsychicCache psychicCache)) return true;
        ...
        // }
        private static bool PreOrdersGiven(List<Pawn> selectedPawns, Vector3 clickPos, List<FloatMenuOption> context)
        {
	        return IsPawnResponsive(selectedPawns[0], true);
        }
